(() => {
    ...
    setInterval(garantirPainel, 1500);
    ...
    document.addEventListener('click', (e) => {
        ...
        if (e.target.id === 'v-leads-iniciar') { if (estadoRobo !== 'RODANDO') { estadoRobo = 'RODANDO'; loopPrincipal(); } }
        if (e.target.id === 'v-leads-pausar') { estadoRobo = 'PAUSADO'; atualizarStatus("⏸️ Pausado", "#ffaa00"); }
        if (e.target.id === 'v-leads-parar') { estadoRobo = 'PARADO'; atualizarStatus("⏹️ Parado", "#ff3333"); }
    });
    ...
    async function executarFluxoZap() {
        try {
            ...
            if (!abaAlvo) { atualizarStatus("✅ Todas as abas concluídas!", "#ff99cc"); estadoRobo = 'PARADO'; return; }
            ...
            if (isAtivo(1)) {
                ...
                const botoesZap = Array.from(document.querySelectorAll('button, .q-btn')).filter(b => b.querySelector('.fa-whatsapp, .fab.fa-whatsapp'));
                let btnZap = botoesZap[botoesZap.length - 1]; 
                if (btnZap) await simularCliqueFirme(btnZap);
                await safeWait(getWait(1, 5000)); 
                if (document.body.innerText.includes('whatsapp ativo')) { temWhatsApp = false; }
            }

            if (temWhatsApp) {
                if (isAtivo(2)) { const clipe = document.querySelector('.fa-paperclip'); if (clipe) { await simularCliqueFirme(clipe.closest('button') || clipe); await safeWait(getWait(2, 3000)); } }
                if (isAtivo(3)) { const btnTemp = document.querySelector('.fa-file-alt'); if (btnTemp) { await simularCliqueFirme(btnTemp.closest('button') || btnTemp); await safeWait(getWait(3, 4000)); } }
                if (isAtivo(4)) { const modal = document.querySelector('.q-dialog'); if (modal) { const seta = modal.querySelector('.fa-caret-down'); if (seta) { await simularCliqueFirme(seta.closest('.q-field') || seta); await safeWait(getWait(4, 3000)); } } }
                ...
                if (isAtivo(5)) {
                    ...
                    if (item) { await simularCliqueFirme(item); await safeWait(getWait(5, 5000)); }
                }
                ...
                if (isAtivo(7)) { const btnS = Array.from(document.querySelectorAll('.q-dialog button')).find(b => b.innerText.includes('ENVIAR')); if (btnS) { simularClique(btnS); await safeWait(getWait(7, 5000)); } }
                if (isAtivo(8)) { const iconesX = Array.from(document.querySelectorAll('.fa-times')); let btnFecharZap = iconesX.find(i => i.closest('.q-toolbar')); if (btnFecharZap) { simularClique(btnFecharZap.closest('button') || btnFecharZap); await safeWait(getWait(8, 3000)); } }
                ...
                if (isAtivo(9)) {
                    ...
                    let coment = document.querySelector('.fa-comment-dots.text-white') || Array.from(document.querySelectorAll('.fa-comment-dots')).pop();
                    if (coment) {
                        await simularCliqueFirme(coment.closest('button, .q-btn') || coment); await safeWait(3000);
                        const campo = document.querySelector('.q-dialog textarea') || document.querySelector('textarea');
                        if (campo) {
                ...
                if (isAtivo(10)) {
                    ...
                    if (tipoAgenda === 'lembrete') {
                        btnAgenda = document.querySelector('a.bg-purple, button.bg-purple') || Array.from(document.querySelectorAll('.fa-clock')).pop();
                    } else {
                        btnAgenda = document.querySelector('a.bg-deep-purple, button.bg-deep-purple') || Array.from(document.querySelectorAll('.fa-calendar, .fa-calendar-alt')).pop();
                    }
                    ...
                    if (btnAgenda) {
                        ...
                        const modalAgenda = document.querySelector('.q-dialog');
                        if (modalAgenda) {
                            ...
                            if (valDataBruta) {
                                ...
                                if (spanData) {
                                    ...
                                    await safeWait(2000); // Espera o menu do calendário abrir
                                    ...
                                    while (tentativas-- > 0) {
                                        ...
                                        if (anoTela < alvoAno || (anoTela === alvoAno && mesIdx < alvoMes)) {
                                            ...
                                            break; // Achou o mês e ano corretos!
                                        }
                                    }
                            ...
                            // LÓGICA DE INSERIR OBSERVAÇÃO (Com native setter para forçar o Quasar a salvar)
                ...
                if (isAtivo(11)) {
                    ...
                    if (btnCheck) { simularClique(btnCheck); await safeWait(2000); const spanOk = Array.from(document.querySelectorAll('span.block')).find(s => s.innerText.trim().toLowerCase() === 'ok'); if (spanOk) { simularClique(spanOk.closest('button')); await safeWait(getWait(11, 3000)); } }
                }
            }
            ...
            if (isAtivo(12)) {
                ...
                if (btnFecharAba) { simularClique(btnFecharAba.closest('div, button, .q-tab') || btnFecharAba); await safeWait(getWait(12, 5000)); }
        ...
        } catch (e) { atualizarStatus(e.message === "STOP" ? "⏹️ Parado." : `Erro: ${e.message}`, "#ff3333"); estadoRobo = 'PARADO'; }
    }
    ...
    async function loopPrincipal() {
        ...
        while (estadoRobo === 'RODANDO' || estadoRobo === 'PAUSADO') {
            if (estadoRobo === 'RODANDO') await executarFluxoZap();
